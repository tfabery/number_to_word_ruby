...
class Fixnum
  define_method(:num_to_word) do
    ...
    new_num = self
    # binding.pry()
    while new_num >= 100000 do
      word.push(num_hash_ones[new_num./(100000)] + " hundred")
      new_num = new_num.%(100000)
    end
    while new_num >= 10000 do
      word.push(num_hash_tens[new_num./(10000)])
      new_num = new_num.%(10000)
    end
    while new_num >= 1000 do
      word.push(num_hash_ones[new_num./(1000)] + " thousand")
      new_num = new_num.%(1000)
    end
    while new_num >= 100 do
      word.push(num_hash_ones[new_num./(100)] + " hundred")
      new_num = new_num.%(100)
    end
    while new_num > 19 do
      word.push(num_hash_tens[new_num./(10)])
      new_num = new_num.%(10)
    end
    while new_num >= 10 do
      word.push(num_hash_tens[new_num])
      new_num = 0
    end
    while new_num > 0 do
      word.push(num_hash_ones[new_num])
      new_num = 0
    end
